...
public class CodeGenerator {
    ...
    private void generateMethodSignature(MethodReference methodRef, boolean isStatic) {
        printType(methodRef.getReturnType());
        writer.print(" ").print(names.forMethod(methodRef)).print("(");
        ...
        if (methodRef.parameterCount() > 0 || !isStatic) {
            ...
            if (!isStatic) {
                ...
                printType(methodRef.parameterType(1));
                writer.print(" local_1");
                firstParam++;
            }
            for (int i = firstParam; i <= methodRef.parameterCount(); ++i) {
                writer.print(", ");
                printType(methodRef.parameterType(i));
                writer.print(" ").print("local_").print(String.valueOf(i));
            }
        }
    ...
    private void generateLocals(RegularMethodNode methodNode) {
        for (int i = methodNode.getReference().parameterCount() + 1; i < methodNode.getVariables().size(); ++i) {
            ...
            printType(variableNode.getType());
            writer.print(" local_").print(String.valueOf(i)).println(";");
        }
    ...
    private void printType(ValueType type) {
    ...
    private void printType(VariableType type) {
        switch (type) {
            case INT:
                writer.print("int32_t");
                break;
